...
- (id)initWithLocation:(CGPoint)location
          ...
          andMaxRating:(NSInteger)maxRating
{
    CGRect fRect = CGRectMake(location.x,
                              location.y,
                              (maxRating * kStarWidthAndHeight),
                              kStarWidthAndHeight);
    if (self = [self initWithFrame:fRect])
    {
        _rating = 0;
        ...
        imageWidth = _solidImage.size.width;
        if (_solidImage.size.width>16) {
            imageWidth +=5;
        }
        
        fRect.size.height = imageWidth;
        fRect.size.width = maxRating * imageWidth;
        [self setFrame:fRect];
    }
